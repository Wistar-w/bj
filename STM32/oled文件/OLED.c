...
//使用之前请在魔术棒的C/C++的Misc Controls里添加--no-multibyte-chars
...
ErrorStatus OLED_CheckDevice(uint8_t slave_addr)
{
    ...
    // 检测通信
    if (HAL_I2C_Master_Transmit(&hi2c2, (slave_addr << 1), &dummy, 0, IIC_TIMEOUT) == HAL_OK)
    {
        return SUCCESS;
    }
...
ErrorStatus OLED_WriteByte(uint8_t cmd, uint8_t byte)
{
    ...
    if (HAL_I2C_Master_Transmit(&hi2c2, OLED_SLAVER_ADDR << 1, buf, 2, IIC_TIMEOUT) == HAL_OK)
...
ErrorStatus OLED_WriteBuffer(uint8_t cmd, uint8_t *buffer, uint32_t num)
{
    ...
    HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(&hi2c2, OLED_SLAVER_ADDR << 1, buf, num + 1, IIC_TIMEOUT);
...
//	OLED_WriteByte(OLED_WR_CMD,0xAE);	//设置显示开启/关闭，0xAE关闭，0xAF开启
//	
//	OLED_WriteByte(OLED_WR_CMD,0xD5);	//设置显示时钟分频比/振荡器频率
//	OLED_WriteByte(OLED_WR_CMD,0x80);	//0x00~0xFF
//	
//	OLED_WriteByte(OLED_WR_CMD,0xA8);	//设置多路复用率
//	OLED_WriteByte(OLED_WR_CMD,0x3F);	//0x0E~0x3F
//	
//	OLED_WriteByte(OLED_WR_CMD,0xD3);	//设置显示偏移
//	OLED_WriteByte(OLED_WR_CMD,0x00);	//0x00~0x7F
//	
//	OLED_WriteByte(OLED_WR_CMD,0x40);	//设置显示开始行，0x40~0x7F
//	
...
//	
//	OLED_WriteByte(OLED_WR_CMD,0x81);	//设置对比度
//	OLED_WriteByte(OLED_WR_CMD,0xCF);	//0x00~0xFF

//	OLED_WriteByte(OLED_WR_CMD,0xD9);	//设置预充电周期
//	OLED_WriteByte(OLED_WR_CMD,0xF1);

//	OLED_WriteByte(OLED_WR_CMD,0xDB);	//设置VCOMH取消选择级别
//	OLED_WriteByte(OLED_WR_CMD,0x30);

//	OLED_WriteByte(OLED_WR_CMD,0xA4);	//设置整个显示打开/关闭

//	OLED_WriteByte(OLED_WR_CMD,0xA6);	//设置正常/反色显示，0xA6正常，0xA7反色

//	OLED_WriteByte(OLED_WR_CMD,0x8D);	//设置充电泵
...
//	X += 2;
	...
	/*通过指令设置页地址和列地址*/
	OLED_WriteByte(OLED_WR_CMD,0xB0 | Page);					//设置页位置
	OLED_WriteByte(OLED_WR_CMD,0x10 | ((X & 0xF0) >> 4));	//设置X位置高4位
	OLED_WriteByte(OLED_WR_CMD,0x00 | (X & 0x0F));			//设置X位置低4位
}
...
void OLED_Update(void)
{
	...
	for (j = 0; j < 8; j ++)
	{
		...
		OLED_WriteBuffer(OLED_WR_DATA,OLED_DisplayBuf[j], 128);
	}
}
...
void OLED_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Page; j < Page1; j ++)
	{
		if (X >= 0 && X <= 127 && j >= 0 && j <= 7)		//超出屏幕的内容不显示
		{
			...
			OLED_WriteBuffer(OLED_WR_DATA,&OLED_DisplayBuf[j][X], Width);
		}
	}
}
